...
@BBOX_ASSIGNERS.register_module()
class MSEHungarianAssigner(BaseAssigner):
    """Computes one-to-one matching between predictions and ground truth for
    mask.
    ...
    def __init__(self,
                 cls_cost=dict(type='ClassificationCost', weight=1.0),
                 mse_cost=dict(type='MSECost', weight=1.0)):
        self.cls_cost = build_match_cost(cls_cost)
        self.mse_cost = build_match_cost(mse_cost)
    ...
    def assign(self,
        ...
        if self.mse_cost.weight != 0:
            # mask_pred shape = [num_query, h, w]
            # gt_mask shape = [num_gt, h, w]
            # mask_cost shape = [num_query, num_gt]
            mse_cost = self.mse_cost(mask_pred, gt_mask)
        else:
            mse_cost = 0

        cost = cls_cost + mse_cost
...
@BBOX_ASSIGNERS.register_module()
class ClsHungarianAssigner(BaseAssigner):
    """Computes one-to-one matching between predictions and ground truth for
    mask.
...
@BBOX_ASSIGNERS.register_module()
class FuseHungarianAssigner(BaseAssigner):
    """Computes one-to-one matching between predictions and ground truth for
    mask.
    ...
    def assign(self,
        ...
        if self.mse_cost.weight != 0:
            ...
            mse_cost = 0
        cost = cls_cost + mask_cost + dice_cost + mse_cost
