...
class SORDataset(CustomDataset):
    CLASSES = ('Salient Object', )
    # PALETTE = [[217, 96, 70], [242, 193, 114], [118, 174, 186], [75, 102, 149],
    #            [85, 86, 116]]
    PALETTE = [[70, 96, 217], [114, 193, 242], [186, 174, 118], [149, 102, 75],
               [116, 86, 85]] + 100*[[116, 86, 85]]
    ...
    def __init__(self,
                 ...
                 rank_prefix=''):
        super(SORDataset, self).__init__(
                 ann_file,
                 pipeline,
                 classes,
                 data_root,
        ...
        if self.data_root is not None:
            if not (self.rank_prefix is None or osp.isabs(self.rank_prefix)):
                self.rank_prefix = osp.join(self.data_root, self.rank_prefix)

    def load_annotations(self, ann_file):
        """

        """
        with open(ann_file, "r") as a:
            info = json.load(a)

        data_infos = [{'filename': info[i]['img']+'.jpg',
                       'object_data': info[i]['object_data'],
                       'width': 640,
                       'height': 480}
    ...
    def get_ann_info(self, idx):
        ...
        rank_file = os.path.join(self.rank_prefix, img_name+'.json')

        with open(rank_file, "r") as r:
            rank_data = json.load(r)

        rank = rank_data["rank_order"]
        rank_level = [(10 - r*10) for r in rank]  # 显著性度转化为显著性等级
        rank_index = np.where(np.array(rank) > self.SAL_VAL_THRESH)[0].tolist()  # 获取显著性目标的位置索引
        bboxes = np.array([object_data[i]['bbox'] for i in rank_index], dtype=np.float32)
        rank_labels = np.array([2*rank[i]-1 for i in rank_index], dtype=np.float32)
        ...
        # labels = np.array([i for i in rank_labels], dtype=np.int64)

        masks = [object_data[i]['segmentation'] for i in rank_index]
    ...
    def evaluate_mae(self, results):
        ...
        for result in results:
            ...
            gt_ranks = len(result['gt_ranks']) - result['gt_ranks']
            pred_ranks = len(result['rank_pred_level']) - result['rank_pred_level']
            pred_ranks = np.array([i for i in pred_ranks], dtype=np.int32)

            post_gt_masks = np.zeros((gt_masks.shape[1], gt_masks.shape[2]))
            for idx, rank in enumerate(gt_ranks):
                post_gt_masks[gt_masks[idx] > 0] = self.RANK_PIXEL[rank] / 255

            post_pred_masks = np.zeros((segmaps.shape[1], segmaps.shape[2]))
            for idx, rank in enumerate(pred_ranks):
                post_pred_masks[segmaps[idx] > 0] = self.RANK_PIXEL[rank] / 255
