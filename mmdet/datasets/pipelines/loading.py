...
@PIPELINES.register_module()
class SORLoadAnnotations(LoadAnnotations):
    """
    ...
    def __init__(self,
        ...
        super(SORLoadAnnotations, self).__init__(
            with_bbox=with_bbox,
        ...
        self.with_rank_label = with_rank_label

    def _load_rank_labels(self, results):
        """Private function to load label annotations.
        ...
        results['rank_labels'] = results['ann_info']['rank_labels'].copy()
        return results
    ...
    def __call__(self, results):
        ...
        if self.with_seg:
            results = self._load_semantic_seg(results)
        if self.with_rank_label:
            results = self._load_rank_labels(results)
        return results
...
class FilterAnnotations:
    ...
    def __call__(self, results):
        ...
        keys = ('gt_bboxes', 'gt_labels', 'gt_masks', 'rank_labels')
        for key in keys:
            if key in results:
