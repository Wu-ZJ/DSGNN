...
def load_checkpoint(
    ...
    state_dict_new_copy = state_dict_new.copy()
    for k in state_dict_new.keys():
        if 'structure' in k and 'rank' not in k:
            k_ = k.replace('structure_', '')
            assert k_ in state_dict.keys()
            state_dict_new_copy[k] = state_dict[k_]

        elif 'texture' in k and 'rank' not in k:
            k_ = k.replace('texture_', '')
            assert k_ in state_dict.keys()
            state_dict_new_copy[k] = state_dict[k_]

        elif k in state_dict.keys():
            state_dict_new_copy[k] = state_dict[k]

        else:
            del state_dict_new_copy[k]
    ...
    state_dict_new_copy._metadata = metadata

    # load state_dict
    load_state_dict(model, state_dict_new_copy, strict, logger)
    return checkpoint
