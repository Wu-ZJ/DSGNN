...
class SORDIIHead(BBoxHead):
    ...
    def __init__(self,
        ...
        assert init_cfg is None, 'To prevent abnormal initialization ' \
                                 'behavior, init_cfg is not allowed to be set'
        super(SORDIIHead, self).__init__(
            num_classes=num_classes,
            reg_decoded_bbox=True,
            reg_class_agnostic=True,
            init_cfg=init_cfg,
    ...
    def init_weights(self):
        """Use xavier initialization for all weight parameter and set
        classification head bias as a specific value when use focal loss."""
        super(SORDIIHead, self).init_weights()
        for p in self.parameters():
            if p.dim() > 1:
                nn.init.xavier_uniform_(p)
            else:
    ...
    def forward(self, roi_feat, proposal_feat):
        ...
        cls_score = self.fc_cls(cls_feat).squeeze(-1).view(N, num_proposals)
        bbox_delta = self.fc_reg(reg_feat).view(N, num_proposals, 4)
    ...
    def loss(self,
        ...
        if cls_score is not None:
            if cls_score.numel() > 0:
                #print(cls_score)
                N, num_proposals = cls_score.shape[:2]
                label_cls = labels.view(N, num_proposals)
                # pos_inds_cls = labels.view(N, num_proposals)
                # label_cls = label_cls[pos_inds_cls.type(torch.bool)]
