...
class MaskFormer(SingleStageDetector):
    ...
    def __init__(self,
        ...
        #self.max_salient_object = self.panoptic_head.max_salient_object
    ...
    def forward_train(self,
        ...
        super(SingleStageDetector, self).forward_train(img, img_metas)
        x = self.extract_feat(img)
        losses = self.panoptic_head.forward_train(x, img, img_metas, gt_bboxes,
                                                  gt_labels, gt_masks, rank_labels,
                                                  gt_semantic_seg,
                                                  gt_bboxes_ignore)
    ...
    def simple_test(self, imgs, img_metas, **kwargs):
        ...
        for i in range(len(results)):
            ...
            # if 'rank_results' in results[i]:
            #     pred_info = results[i]['rank_results']
            #     bboxes, mask_rank = pred_info['bboxes'], pred_info['mask_rank']
            #     mask_pred_binary = pred_info['mask_pred_binary']
            #     bbox_results = bbox2result(bboxes, mask_rank, self.max_salient_object) #bbox_results:按预测的显著性，由高到低排序
            #
            #     mask_results = [[] for _ in range(self.max_salient_object)]
            #     for j, label in enumerate(mask_rank):
            #         mask = mask_pred_binary[j].detach().cpu().numpy()
            #         mask_results[int(label)].append(mask)
            #     results[i]['rank_post_results'] = bbox_results, mask_results
