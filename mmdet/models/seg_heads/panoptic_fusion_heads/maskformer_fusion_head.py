...
class MaskFormerFusionHead(BasePanopticFusionHead):
    ...
    def instance_postprocess(self, mask_cls, mask_pred):            # 后处理操作，输入是预测的得分最高的目标以及分割图
        """Instance segmengation postprocess.                       # 应当返回预测显著性排名结果以及相应的位置框
...
@HEADS.register_module()
class MaskFormerSORFusionHead(BasePanopticFusionHead):
    ...
    def salience_ranking_postprocess(self, mask_score, mask_pred):
        """
        ...
        """
        # max_salient_object = self.test_cfg.get('max_salient_object', 10)
        # _, indices = mask_score.topk(max_salient_object, sorted=True)                      #分数越高，显著性应该越靠后, mask_rank是分数越高数值越小， rank_pred才是分数越高对应的数值越大
        #
        # mask_rank = torch.zeros(max_salient_object, device=mask_score.device)
        # for rank, indice in enumerate(indices):
        #     mask_rank[indice] = rank

        # rank_pred = (len(mask_rank) - mask_rank)
        rank_pred = np.array([sorted(mask_score).index(a) + 1 for a in mask_score], dtype=np.float32)
        mask_pred_binary = (mask_pred > 0).float()
        mask_pred_binary = mask_pred_binary.bool()
        bboxes = mask2bbox(mask_pred_binary)
        bboxes = torch.cat([bboxes, mask_score[:, None]], dim=-1)
        result = dict(
            mask_score=mask_score.detach().cpu().numpy(),
            rank_pred=rank_pred,
            bboxes=bboxes,
    ...
    def simple_test(self,
        ...
        semantic_on = self.test_cfg.get('semantic_on', False)
        instance_on = self.test_cfg.get('instance_on', False)
        ranking_on = self.test_cfg.get('ranking_on', True)
        assert not semantic_on, 'segmantic segmentation '\
            'results are not supported yet.'
        ...
        for mask_cls_result, mask_pred_result, meta in zip(
            ...
            if ranking_on:
                rank_results = self.salience_ranking_postprocess(
                    mask_cls_result, mask_pred_result)
