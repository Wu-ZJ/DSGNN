...
class GNNLayer(nn.Module):
    def __init__(self, in_channels, num_classes, norm_cfg=dict(type='LN')):
        ...
        self.U = nn.Linear(dim_in, dim_out, bias=True)
        self.V = nn.Linear(dim_in, dim_out, bias=True)
        ...
        self.bnv = build_norm_layer(norm_cfg, dim_out)[1]
        self.bne = build_norm_layer(norm_cfg, dim_out)[1]
...
#         self.norm1 = build_norm_layer(dict(type='LN'), in_dim)[1]
#
#         self.gnn = GNN(in_dim, num_classes, layer_num=3)
#         self.norm2 = build_norm_layer(dict(type='LN'), in_dim)[1]
#
...
#         n_att = self.norm1(n_res + node)
#
#         node, edge = self.gnn(n_att, edge)
#         node = self.norm2(n_att + node)
#
...
class GNNEncoder(nn.Module):
    def __init__(self, in_dim, num_classes, norm_cfg=dict(type='GN', num_groups=32)):
        ...
        self.gnn = GNN(in_dim, num_classes, layer_num=2)
        self.ffm_node = FFM(in_dim, in_dim)
    ...
    def forward(self, x):
        ...
        edge = self.edge_extractor(x, global_feature)

        node, edge = self.gnn(x, edge)
