...
from .sor_gated_gcn7 import DomainSepModule, DomainSepModule2


@HEADS.register_module()
class Mask2FormerGNNHead9(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        self.rank_head = DomainSepModule(feat_channels, num_queries) #TODO
    ...
    def node_loss(self, rank_results, mask_preds, gt_labels_list, rank_labels_list, gt_masks_list, img_metas):
        ...
        (labels_lf_list, label_weights_lf_list,
         ...
         num_total_lf_pos, num_total_lf_neg) = \
            self.get_targets(cls_pred_lf_list, mask_preds_list,
                             gt_labels_list, rank_labels_list, gt_masks_list, img_metas)
        ...
        (labels_hf_list, label_weights_hf_list,
         ...
         num_total_hf_pos, num_total_hf_neg) = \
            self.get_targets(cls_pred_hf_list, mask_preds_list,
                             gt_labels_list, rank_labels_list, gt_masks_list, img_metas)
    ...
    @force_fp32(apply_to=('structure_cls_scores', 'structure_rank_scores', 'structure_mask_preds',
                          'texture_cls_scores', 'texture_rank_scores', 'texture_mask_preds', 'rank_labels_list', 'img'))
    ...
    def forward(self, feats, img_metas):
        ...
        rank_results = self.forward_gnn_sep_head(structure_query_feat, texture_query_feat)
    ...
    def simple_test(self, feats, img_metas, **kwargs):
        ...
        for cp in cls_pred:
            ...
            salient_obj[scores_per_image > self.score_thr] = 1
