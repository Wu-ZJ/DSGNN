...
class Mask2FormerGNNHead5(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if train_cfg:
            ...
            self.mask_assigner = build_assigner(self.train_cfg.rank_assigner)
            self.mask_sampler = build_sampler(self.train_cfg.rank_sampler, context=self)
    ...
    def get_mask_targets(self, mask_preds_list, gt_masks_list, img_metas):

        (mask_targets_list, mask_weights_list,
         pos_inds_list, neg_inds_list) = \
            multi_apply(self._get_mask_target_single,
                        mask_preds_list, gt_masks_list, img_metas)

        num_total_pos = sum((inds.numel() for inds in pos_inds_list))
        num_total_neg = sum((inds.numel() for inds in neg_inds_list))

        return (mask_targets_list, mask_weights_list,
                num_total_pos, num_total_neg)

    def _get_mask_target_single(self, mask_pred, gt_masks, img_metas):
        # sample points
        ...
        # assign and sample
        assign_result = self.mask_assigner.assign(mask_points_pred,
                                                  gt_points_masks,
                                                  img_metas)
        sampling_result = self.mask_sampler.sample(assign_result,
                                                   mask_pred,
                                                   gt_masks)
        pos_inds = sampling_result.pos_inds
        neg_inds = sampling_result.neg_inds
    ...
    def structure_loss_single(self, mask_preds, gt_masks_list, img_metas):
        ...
        (mask_targets_list, mask_weights_list,
         num_total_pos, num_total_neg) = \
            self.get_mask_targets(mask_preds_list, gt_masks_list, img_metas)
        ...
        num_total_masks = reduce_mean(mask_preds.new_tensor([num_total_pos]))
    ...
    def texture_loss_single(self, mask_preds_exp, mask_preds,
        ...
        """
        num_imgs = mask_preds.size(0)
        mask_preds_list = [mask_preds_exp[i] for i in range(num_imgs)]
        ...
        # shape (num_total_gts, h, w)
        mask_targets_list = self.get_texture_labels(img_list, mask_targets_list)
        mask_targets = torch.cat(mask_targets_list, dim=0)
        # shape (batch_size, num_queries)
        mask_weights = torch.stack(mask_weights_list, dim=0)
    ...
    def node_loss(self, rank_results, mask_preds, gt_labels_list, rank_labels_list, gt_masks_list, img_metas): #TODO
        ...
        (labels_list, label_weights_list,
         ...
         mask_targets_list, mask_weights_list,
         num_total_pos, num_total_neg) = \
            self.get_targets(cls_pred_list, mask_preds_list,
                             gt_labels_list, rank_labels_list, gt_masks_list, img_metas)
    ...
    @force_fp32(apply_to=('structure_mask_preds', 'texture_mask_preds', 'rank_results'))
    def loss(self, structure_mask_preds, texture_mask_preds, rank_results_list,
        ...
        num_dec_layers = len(structure_mask_preds)
        ...
        losses_structure_mask, losses_structure_dice = multi_apply(
            self.structure_loss_single, structure_mask_preds,
            all_gt_masks_list, img_metas_list)

        losses_texture_mse, = multi_apply(
            self.texture_loss_single, structure_mask_preds, texture_mask_preds,
            all_gt_masks_list, img_list, img_metas_list)
        ...
        num_dec_layer = 0
        for loss_st_mask_i, loss_st_dice_i, loss_te_mse_i, loss_domain_i in zip(
                losses_structure_mask[:-1], losses_structure_dice[:-1],
                losses_texture_mse[:-1], losses_domain[:-1]):
    ...
    def forward_texture_head(self, decoder_out, mask_feature, attn_mask_target_size):
        ...
        return torch.tanh(mask_pred)

    def forward_gnn_sep_head(self, structure_feat, texture_feat):
        structure_feat = self.structure_transformer_decoder.post_norm(structure_feat)
        texture_feat = self.texture_transformer_decoder.post_norm(texture_feat)

        structure_feat = structure_feat.transpose(0, 1)
        texture_feat = texture_feat.transpose(0, 1)
    ...
    def forward(self, feats, img_metas):
        ...
        return structure_mask_pred_list, texture_mask_pred_list, rank_results_list
    ...
    def forward_train(self,
        ...
        # forward
        structure_mask_pred_list, texture_mask_pred_list, rank_results_list = self(feats, img_metas)
        ...
        # loss
        losses = self.loss(structure_mask_pred_list, texture_mask_pred_list,
...
class GEM(nn.Module):
    def __init__(self, in_channels, num_classes, norm_cfg=dict(type='SyncBN')):
        super(GEM, self).__init__()
        self.FAM = nn.TransformerDecoderLayer(in_channels, nhead=8,
                                              dim_feedforward=4*in_channels,
                                              dropout=0., batch_first=True)

        self.ARM = nn.TransformerDecoderLayer(in_channels, nhead=8,
                                              dim_feedforward=4*in_channels,
                                              dropout=0., batch_first=True)
...
class DomainSepModule(nn.Module):
    def __init__(self, in_dim, num_classes, norm_cfg=dict(type='GN', num_groups=32)):
        ...
        self.rank_embed_hf = FFM(in_dim, in_dim)
        self.rank_embed_lf = FFM(in_dim, in_dim)
        ...
        self.rank_embed = FFM(in_dim * 2, in_dim)
        # self.restore_hf = FFM(in_dim * 2, in_dim)
        # self.restore_lf = FFM(in_dim * 2, in_dim)
    ...
    def forward(self, structure_feature, texture_feature):
        structure_feature = self.rank_embed_hf(structure_feature)
        texture_feature = self.rank_embed_lf(texture_feature)
        ...
        restored_hf_feature = self.shared_gnn_dec(restored_hf_feature)
