...
class Mask2FormerGNNHead10(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        self.rank_head = DomainSepModule(feat_channels, self.max_salient_object)
    ...
    def _get_target_single(self, cls_score, mask_pred,
        ...
        # rank target
        ranks = rank_labels.new_full((num_queries, ),
                                     0,
                                     dtype=torch.float32)
        ranks[pos_inds] = rank_labels[sampling_result.pos_assigned_gt_inds]
        rank_weights = rank_labels.new_ones((num_queries, ))
    ...
    def structure_loss_single(self, cls_scores, mask_preds,
                              gt_labels_list, rank_labels_list,
                              gt_masks_list, img_metas):
    ...
    def texture_loss_single(self, cls_scores, mask_preds_exp, mask_preds, #TODO
                            gt_labels_list, rank_labels_list,
    ...
    @force_fp32(apply_to=('structure_cls_scores', 'structure_mask_preds', 'texture_cls_scores', 'texture_mask_preds',
                          'rank_results_list', 'structure_salient_mask_preds'))
    def loss(self, structure_cls_scores, structure_mask_preds, texture_cls_scores, texture_mask_preds,
             rank_results_list, structure_salient_mask_preds,
        ...
        losses_structure_cls, losses_structure_mask, losses_structure_dice = multi_apply(
            self.structure_loss_single, structure_cls_scores, structure_mask_preds,
            all_gt_labels_list, all_rank_labels_list, all_gt_masks_list, img_metas_list)

        losses_texture_cls, losses_texture_mse = multi_apply(
            self.texture_loss_single, texture_cls_scores, structure_mask_preds, texture_mask_preds,
            all_gt_labels_list, all_rank_labels_list, all_gt_masks_list, img_list, img_metas_list)

        losses_domain, = multi_apply(
            self.node_loss, rank_results_list, structure_salient_mask_preds,
            all_gt_labels_list, all_rank_labels_list, all_gt_masks_list, img_metas_list
        )
        ...
        num_dec_layer = 0
        for loss_st_cls_i, loss_st_mask_i, loss_st_dice_i, \
            loss_te_cls_i, loss_te_mse_i, loss_domain_i in zip(
                losses_structure_cls[:-1], losses_structure_mask[:-1], losses_structure_dice[:-1],
                losses_texture_cls[:-1], losses_texture_mse[:-1], losses_domain[:-1]):
    ...
    def forward_mask2former(self, feats, img_metas):
        ...
        structure_query_feat_list = []
        texture_cls_pred_list = []
        texture_mask_pred_list = []
        texture_query_feat_list = []

        structure_cls_pred, structure_mask_pred, attn_mask = self.forward_structure_head(
            structure_query_feat, mask_features, multi_scale_memorys[0].shape[-2:])

        texture_cls_pred, texture_mask_pred = self.forward_texture_head(
        ...
        structure_query_feat_list.append(structure_query_feat.transpose(0, 1))
        texture_cls_pred_list.append(texture_cls_pred)
        texture_mask_pred_list.append(texture_mask_pred)
        texture_query_feat_list.append(texture_query_feat.transpose(0, 1))
        ...
        return structure_cls_pred_list, structure_query_feat_list, structure_mask_pred_list, \
               texture_cls_pred_list, texture_query_feat_list, texture_mask_pred_list

    def forward(self, feats, img_metas):
        structure_cls_pred_list, structure_query_feat_list, structure_mask_pred_list, \
        texture_cls_pred_list, texture_query_feat_list, texture_mask_pred_list = \
            self.forward_mask2former(feats, img_metas)

        len_dec_layers = len(structure_cls_pred_list)

        rank_results_list = []
        st_pred_salient_masks = []
        te_pred_salient_masks = []
        all_salient_objs = []
        for i in range(len_dec_layers):
            # (batch_size, num_queries, cls_out_channels) (batch_size, num_queries, h, w)
            structure_cls_pred, structure_query_feat, structure_mask_pred, \
            texture_cls_pred, texture_query_feat, texture_mask_pred = \
            structure_cls_pred_list[i], structure_query_feat_list[i], structure_mask_pred_list[i], \
            texture_cls_pred_list[i], texture_query_feat_list[i], texture_mask_pred_list[i]
            ...
            text_salient_mask = []
            batch_salient_indx = []
            for st_cls_pred, st_mask_pred, st_node_feat, te_cls_pred, te_mask_pred, te_node_feat \
                    in zip(structure_cls_pred, structure_mask_pred, structure_query_feat,
                           texture_cls_pred, texture_mask_pred, texture_query_feat):

                st_scores = F.softmax(st_cls_pred, dim=-1)[:, :-1]
                te_scores = F.softmax(te_cls_pred, dim=-1)[:, :-1]

                st_scores_per_image, st_top_indices = st_scores.flatten(0, 1).topk(
                    self.max_salient_object, sorted=False)
                te_scores_per_image, te_top_indices = te_scores.flatten(0, 1).topk(
                    self.max_salient_object, sorted=False)

                salient_obj = torch.zeros(self.max_salient_object).to(st_scores_per_image.device) # TODO
                salient_obj[st_scores_per_image > 0.80] = 1
                batch_salient_indx.append(salient_obj)

                st_salient_node = st_node_feat[st_top_indices]
                te_salient_node = te_node_feat[te_top_indices]

                st_salient_mask = st_mask_pred[st_top_indices]
                te_salient_mask = te_mask_pred[te_top_indices]

                struc_salient_node.append(st_salient_node)
                struc_salient_mask.append(st_salient_mask)
                text_salient_node.append(te_salient_node)
                text_salient_mask.append(te_salient_mask)

            struc_salient_node = torch.stack(struc_salient_node, dim=0)
            struc_salient_mask = torch.stack(struc_salient_mask, dim=0)
            text_salient_node = torch.stack(text_salient_node, dim=0)
            text_salient_mask = torch.stack(text_salient_mask, dim=0)
            batch_salient_indx = torch.stack(batch_salient_indx, dim=0)

            rank_results = self.forward_gnn_sep_head(struc_salient_node, text_salient_node)

            rank_results_list.append(rank_results)
            st_pred_salient_masks.append(struc_salient_mask)
            te_pred_salient_masks.append(text_salient_mask)
            all_salient_objs.append(batch_salient_indx)
        return structure_cls_pred_list, structure_mask_pred_list, texture_cls_pred_list, texture_mask_pred_list, \
               rank_results_list, st_pred_salient_masks, te_pred_salient_masks, all_salient_objs
    ...
    def forward_train(self,
        ...
        structure_cls_pred_list, structure_mask_pred_list, texture_cls_pred_list, texture_mask_pred_list, \
        rank_results_list, st_pred_salient_masks, te_pred_salient_masks, _ = self(feats, img_metas)
        ...
        # loss
        losses = self.loss(structure_cls_pred_list, structure_mask_pred_list,
                           texture_cls_pred_list, texture_mask_pred_list,
                           rank_results_list, st_pred_salient_masks,
    ...
    def simple_test(self, feats, img_metas, **kwargs):
        ...
        structure_cls_pred_list, structure_mask_pred_list, texture_cls_pred_list, texture_mask_pred_list, \
        rank_results_list, st_pred_salient_masks, te_pred_salient_masks, all_salient_objs = self(feats, img_metas)

        # rank_pred = structure_rank_pred_list[-1]
        rank_pred = rank_results_list[-1]['rank_pred_lf']
        cls_pred = rank_results_list[-1]['cls_pred']

        mask_pred_results = st_pred_salient_masks[-1]
        # upsample masks
        ...
        #     salient_obj[scores_per_image > 0.05] = 1
        #     batch_salient_indx.append(salient_obj)
        ...
        # import cv2
        # text_pred_results = te_pred_salient_masks[-1]
...
class GNNEncoder(nn.Module):
    def __init__(self, in_dim, num_classes, norm_cfg=dict(type='SyncBN')):
        ...
        self.num_classes = num_classes
        self.layers = GNN_EDGE(in_dim, self.num_classes)
...
class GraphSepModule(nn.Module):
    def __init__(self, in_dim, num_classes, norm_cfg=dict(type='GN', num_groups=8)):
        super(GraphSepModule, self).__init__()
        self.squ = LinearBlock(in_dim, 32)
        self.enc = GNNEncoder(in_dim=32, num_classes=num_classes, norm_cfg=norm_cfg)
...
class DomainSepModule(nn.Module):
    def __init__(self, in_dim, num_classes, norm_cfg=dict(type='GN', num_groups=32)):
        ...
        self.gnn_sep = GraphSepModule(in_dim=in_dim, num_classes=num_classes)
